...
public struct PropertyAppearanceInContainerRule<T: UIAppearance, Value, Container>: AppearanceRuleProtocol {
    @Reversible private var value: Value
    var container: Container?
    let keypath: WritableKeyPath<T, Value>
    
    public init(keypath: WritableKeyPath<T, Value>, value: Value, container: Container? = nil) {
        self.keypath = keypath
        var proxy: T
        if let container = container {
            proxy = T.appearance(whenContainedInInstancesOf: [container as! UIAppearanceContainer.Type])
        } else {
            proxy = T.appearance()
        }
